...
{
    // Create vertices
    graph::Vertex<double_t, double_t, 2> vertexA({ 0.0, 0.0 }, 0);
    graph::Vertex<double_t, double_t, 2> vertexB({ 1.0, 1.0 }, 1);

    // Create an edge with default cost
    graph::Edge<double_t, double_t, 2> edgeAB(&vertexA, &vertexB);
    ...
    graph::Edge<double_t, double_t, 2> edgeBA(&vertexB, &vertexA, 10);
...
{
    graph::Vertex<int32_t, double_t, 2> vertexA({ 0.0, 0.0 }, 0);
    graph::Vertex<int32_t, double_t, 2> vertexB({ 1.0, 1.0 }, 1);

    graph::Edge<int32_t, double_t, 2> edgeAB(&vertexA, &vertexB);
    graph::Edge<int32_t, double_t, 2> edgeBA(&vertexB, &vertexA, 10);
...
TEST_CASE("Test graph::Edge copy constructor and assignment operator")
{
    ...
    {
        graph::Edge<int32_t, double_t, 2> edgeCopy(edgeAB);
    ...
    {
        graph::Edge<int32_t, double_t, 2> edgeAssignment(&vertexB, &vertexA, 10);
